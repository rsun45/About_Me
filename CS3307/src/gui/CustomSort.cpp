    ...
    if (ui->sort_name->text()!="") {
        if (fieldBoxes.at(0)->currentText()!="") {
            if (!fieldBoxes.at(0)->currentText().contains("Date")) {
               if (!(fieldBoxes.at(0)->currentText().contains("Division") && fieldBoxes.at(1)->currentText() == "")) {
                   ...
                   QMessageBox::critical(this, "", "Sorry, must have a second sort field if Division is the first sort field.");
               }
            } else {
